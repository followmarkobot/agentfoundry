...
export default function DashboardControls({
  ...
  const filtered = useMemo(() => {
    let result = [...repos];
    if (!showArchived) result = result.filter((r) => !prefs.archived.includes(r.id));
    if (hasSearch) {
    ...
    if (selectedLanguages.size > 0) result = result.filter((r) => r.language && selectedLanguages.has(r.language));
    if (filterHasDescription) result = result.filter((r) => r.description && r.description.trim().length > 0);
    if (filterHasStars) result = result.filter((r) => r.stargazers_count > 0);
    if (filterVisibility === "public") result = result.filter((r) => !(r as Record<string, unknown>).private);
    else if (filterVisibility === "private") result = result.filter((r) => (r as Record<string, unknown>).private);
    ...
    result.sort((a, b) => {
      ...
      switch (sort) {
        ...
        case "created": return new Date((b as Record<string, unknown>).created_at as string || b.updated_at).getTime() - new Date((a as Record<string, unknown>).created_at as string || a.updated_at).getTime();
        default: return 0;
      }
    });
    return result;
        ...
        <div className="mt-3 flex flex-wrap items-center gap-2">
          ...
          {languages.map((lang) => (
            <button key={lang} onClick={() => toggleLanguage(lang)}
              className={`rounded-full px-3 py-1 text-xs font-medium border transition ${
                selectedLanguages.has(lang) ? "border-green-400 bg-green-50 text-green-700" : "border-zinc-200 bg-zinc-50 text-zinc-600 hover:border-zinc-300"
              }`}>
              {lang}
            </button>
          ))}
          <span className="mx-1 h-4 w-px bg-zinc-200" />
          <button onClick={() => setFilterHasDescription(!filterHasDescription)}
            className={`rounded-full px-3 py-1 text-xs font-medium border transition ${
              filterHasDescription ? "border-green-400 bg-green-50 text-green-700" : "border-zinc-200 bg-zinc-50 text-zinc-600 hover:border-zinc-300"
            }`}>Has description</button>
          <button onClick={() => setFilterHasStars(!filterHasStars)}
            className={`rounded-full px-3 py-1 text-xs font-medium border transition ${
              filterHasStars ? "border-green-400 bg-green-50 text-green-700" : "border-zinc-200 bg-zinc-50 text-zinc-600 hover:border-zinc-300"
            }`}>Has stars ★</button>
          <button onClick={() => setFilterVisibility(filterVisibility === "all" ? "public" : filterVisibility === "public" ? "private" : "all")}
            className={`rounded-full px-3 py-1 text-xs font-medium border transition ${
              filterVisibility !== "all" ? "border-green-400 bg-green-50 text-green-700" : "border-zinc-200 bg-zinc-50 text-zinc-600 hover:border-zinc-300"
            }`}>
          ...
          {archivedCount > 0 && (
            <button onClick={() => setShowArchived(!showArchived)}
              className={`rounded-full px-3 py-1 text-xs font-medium border transition ${
                showArchived ? "border-amber-400 bg-amber-50 text-amber-700" : "border-zinc-200 bg-zinc-50 text-zinc-600 hover:border-zinc-300"
              }`}>
